...
public final class Operation extends Statement
{
    ...
    @Override
    public final CodeFragmentType getCodeFragmentType() { return CodeFragmentType.OPERATION; }
    ...
    public static final Statement assignment(Operator operator, Statement location, Statement op) throws CompilerError
    {
        ...
        if(!location.isIdentifier())
        {
            if(!location.isOperation())
                throw new CompilerError("Expected valid identifier, array access, property access or peek in left assignment operator. But found: " + location);
            Operation lop = (Operation) location;
            switch(lop.getOperatorSymbol())
            {
                case ARRAY_GET: case PROPERTY_GET: case PEEK: break;
                default:
                    throw new CompilerError("Expected valid identifier, array access, property access, or peek in left assignment operator. But found: " + location);
            }
        }
    ...
    public static final Statement peek(Statement container)
    {
        return new Operation(Operator.PEEK, container);
    }
    ...
    private static Statement literalUnary(Operator operator, Literal value)
    {
        switch(operator.getSymbol())
        {
            ...
            case CAST_BOOLEAN: return value.operatorCastBoolean();
            case CAST_STRING: return value.operatorCastString();
            case CAST_CONST_TUPLE: return value.operatorCastConstTuple();
            case CAST_CONST_MAP: return value.operatorCastConstMap();
            case CAST_FUNCTION: return value.operatorCastFunction();
