...
import kp.gsl.exception.ErrorList;
import kp.gsl.lang.GSLDataType.Typename;
...
public final class CodeParser
{
            ...
            for(;;)
            {
                ...
                switch(c)
                {
                    ...
                    case '(': {
                        ...
                        if(!accumulated.hasLast() || accumulated.last().isReturnBasedCode() ||
                                (!accumulated.last().isStatement() && !accumulated.last().isCommand())) //Parenthesis
                        {
                            ...
                            if(list.count(Stopchar.COMMA) > 0)
                                return accumulated.enqret(Mutable.tuple(list));
                    ...
                    case '[': {
                        ...
                        CodeReader scopeSource = extractScope(source, '[', ']');
                        CodeFragmentList list = parseSubStatement(scopeSource, errors);
                        if(!accumulated.hasLast()|| !accumulated.last().isStatement()) //Literal List or Literal Map
                            return accumulated.enqret(Mutable.tryMapOrList(list));
                    ...
                    case '{': {
                        builder.flush();
                        CodeReader scopeSource = extractScope(source, '{', '}');
                        if(!accumulated.hasLast() || !accumulated.last().isStatement()) // Object, Struct, Blueprint
                        {
                            CodeFragmentList list = parseSubStatement(scopeSource, errors);
                            if(accumulated.hasLast() && accumulated.last().isDataType())
                            {
                                var type = (DataType) accumulated.last();
                                if(type == DataType.OBJECT)
                                {
                                    accumulated.replaceListLast(Mutable.object(list));
                                    return accumulated.dequeue();
                                }
                                if(type == DataType.STRUCT)
                                {
                                    accumulated.replaceListLast(Mutable.struct(list));
                                    return accumulated.dequeue();
                                }
                                if(type == DataType.BLUEPRINT)
                                {
                                    accumulated.replaceListLast(Mutable.blueprint(list));
                                    return accumulated.dequeue();
                                }
                            }
    ...
    private static final class CodeFragmentBuilder
    {
        ...
        public final CodeFragment decode() throws CompilerError
        {
            ...
            switch(str = sb.toString())
            {
                ...
                case Typename.INT: return DataType.INTEGER;
                case Typename.FLOAT: return DataType.FLOAT;
                case Typename.BOOL: return DataType.BOOLEAN;
                case Typename.STRING: return DataType.STRING;
                case Typename.CONST_TUPLE: return DataType.STRING;
                case Typename.CONST_MAP: return DataType.STRING;
                case Typename.CALLABLE: return DataType.STRING;
                case Typename.LIST: return DataType.STRING;
                case Typename.TUPLE: return DataType.STRING;
                case Typename.MAP: return DataType.STRING;
                case Typename.STRUCT: return DataType.STRING;
                case Typename.BLUEPRINT: return DataType.STRING;
                case Typename.OBJECT: return DataType.STRING;
                case Typename.ITERATOR: return DataType.STRING;
                case Typename.BYTES: return DataType.STRING;
                case Typename.NATIVE: return DataType.STRING;
                ...
                case "class": return Operator.BASE;
                case "var": return Command.VAR;
    ...
    private static final class CodeQueue
    {
        ...
        public final CodeFragment removeListLast() { return list.removeLast(); }
        public final CodeQueue replaceListLast(CodeFragment last) { return list.removeLast(); list.add(last); return this; }
